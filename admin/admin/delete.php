...
if(isset($_GET['id'])){
	...
	if(mysqli_num_rows($cek) > 0){
		...
		$del = mysqli_query($koneksi, "DELETE FROM user_level WHERE id='$Id'") or die(mysqli_error($koneksi));
		if($del){
		...
		echo '<script>alert("ID tidak ditemukan di database."); document.location="index.php?page=tampil_user";</script>';
	}
}else{
	echo '<script>alert("ID tidak ditemukan di database."); document.location="index.php?page=tampil_user";</script>';
}
